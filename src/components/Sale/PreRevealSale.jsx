...
export default function PreRevealSale({
  pricePerToken,
  mintedTokens,
  maxCanOwn,
  balance,
  ...
  for (let i = 1; i <= canMint; i++) {
    options.push(
      <option value={i} key={`mint_option_${i}`}>
        {i}
      </option>
    );
  }
  ...
  if (options.length > 0) {
    block = (
      <form onSubmit={(e) => preRevealMint(e)} name="mint-form">
        <span>Mint</span>
        <select name={"mint_amount"}>{options}</select>
  ...
  return (
    <>
      <p className="title">
        Blind Minting Phase: {MAX_TOTAL_SUPPLY - totalSupply} Available
      </p>
      {block}
      {saleHeader}
      <PriceChart />
